  ...
  class TCallbackButton {
    public:
      ...
      TCallbackButton(uint8_t pin,
                      ...
                      uint8_t id = autoId())
      :button_(pin), id_(id), pressed_(false), onPressed_(onPressed), onReleased_(onReleased) {}
      ...
      void handle() {
        button_.handle();
        if(!pressed_ && button_.isPressed()) {
          pressed_ = true;
          if(onPressed_) {
            onPressed_(id_);
          }
        } else if(pressed_ && !button_.isPressed()) {
          pressed_ = false;
          if(onReleased_) {
            onReleased_(id_);
          }
